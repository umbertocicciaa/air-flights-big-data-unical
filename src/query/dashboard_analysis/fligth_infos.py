...
def calculate_on_time_flights(df: DataFrame):
    return df.filter((df["Cancelled"] == 0) & (df["Diverted"] == 0) & (df["ArrDelayMinutes"] == 0)).count()


def count_cancelled_flights(df: DataFrame):
    return df.filter(df["Cancelled"] != 0).count()


def count_diverted_flights(df: DataFrame):
    return df.filter(df["Diverted"] != 0).count()


def calculate_delayed_flights(df: DataFrame):
    return df.filter((df["Cancelled"] == 0) & (df["Diverted"] == 0) & (df["ArrDelayMinutes"] > 0)).count()


def monthly_flight_statistics(df: DataFrame):
