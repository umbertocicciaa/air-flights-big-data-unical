...
def clustering_flights_on_time_delayed(df: DataFrame, num_cluster: int, columns: list, columns_clustering: list):
    assembler = VectorAssembler(
        inputCols=columns_clustering,
        outputCol="features")
    df_features = assembler.transform(df)

    kmeans = KMeans(k=num_cluster, seed=1, featuresCol="features", predictionCol="cluster")
